...
os.environ[
    'TEMP_JSON_DATA'] = '{"subscribes":[{"url":"URL","tag":"tag_1","enabled":true,"emoji":1,"prefix":"","User-Agent":"v2rayng"},{"url":"URL","tag":"tag_2","enabled":false,"emoji":0,"prefix":"❤️","User-Agent":"clashmeta"}],"auto_set_outbounds_dns":{"proxy":"","direct":""},"save_config_path":"./config.json","auto_backup":false,"exclude_protocol":"ssr","config_template":"","Only-nodes":false}'
    ...
    if request.method == 'POST':
        try:
            ...
            if new_temp_json_data:
                ...
                return jsonify({'status': 'error', 'message': 'TEMP_JSON_DATA 不能为空(không thể trống)'},
                               content_type='application/json; charset=utf-8')  # 返回错误状态和消息
        except Exception as e:
...
def config(url):
    ...
    temp_json_data = json.loads(
        '{"subscribes":[{"url":"URL","tag":"tag_1","enabled":true,"emoji":1,"prefix":"","User-Agent":"v2rayng"},{"url":"URL","tag":"tag_2","enabled":false,"emoji":0,"prefix":"❤️","User-Agent":"clashmeta"},{"url":"URL","tag":"tag_3","enabled":false,"emoji":1,"prefix":"","User-Agent":"v2rayng"}],"auto_set_outbounds_dns":{"proxy":"","direct":""},"save_config_path":"./config.json","auto_backup":false,"exclude_protocol":"ssr","config_template":"","Only-nodes":false}')
    ...
    if index_of_colon != -1:
        ...
        if next_char_index < len(encoded_url) and encoded_url[next_char_index] != "/":
            if '/&' in encoded_url:
                encoded_url = encoded_url[:next_char_index - 1] + "/" + encoded_url[
                                                                        next_char_index - 1:encoded_url.find("/&")]
            else:
    ...
    try:
        ...
        subprocess.check_call(
            [sys.executable, 'main.py', '--template_index', selected_template_index, '--temp_json_data',
             temp_json_data])
        ...
        os.environ['TEMP_JSON_DATA'] = json.dumps(json.loads(
            '{"subscribes":[{"url":"URL","tag":"tag_1","enabled":true,"emoji":1,"prefix":"","User-Agent":"v2rayng"},{"url":"URL","tag":"tag_2","enabled":false,"emoji":0,"prefix":"❤️","User-Agent":"clashmeta"}],"auto_set_outbounds_dns":{"proxy":"","direct":""},"save_config_path":"./config.json","auto_backup":false,"exclude_protocol":"ssr","config_template":"","Only-nodes":false}'),
        ...
        return Response(json.dumps({'status': 'error',
                                    'message_CN': '订阅解析超时: 请检查订阅链接是否正确 or 请更换为no_groups模板 再尝试一次; 请不要修改 tag 值，除非你明白它是干什么的;',
